...
def get_apollo_person_details(apollo_person_id: str) -> Optional[dict]:
    apollo_api_key = os.getenv("APOLLO_API_KEY")
    if not apollo_api_key: raise HTTPException(500, "APOLLO_API_KEY not configured")
    try:
        response = requests.get(f"https://api.apollo.io/v1/people/{apollo_person_id}", headers={"X-Api-Key": apollo_api_key, 'Content-Type': 'application/json'}, timeout=15)
        response.raise_for_status()
        return response.json()
    except requests.exceptions.RequestException as e:
        print(f"Error fetching person details from Apollo: {e}")
        return None

def analyze_russia_ties(person_data: dict, company_data: dict) -> Dict:
    # (Your existing analyze_russia_ties function remains unchanged)
...
def generate_outreach_message(person_data: dict, company_data: dict) -> Dict:
    # (Your existing generate_outreach_message function remains unchanged)
    gemini_api_key = os.getenv("GEMINI_API_KEY")
    if not gemini_api_key: raise HTTPException(500, "GEMINI_API_KEY not configured")
    ...
    prompt = f"""You are a business development manager for a Ukrainian firm managing upstream oil and gas assets. Craft a personalized outreach email to the following individual to initiate a conversation about potential PARTNERSHIPS in Ukraine's oil and gas sector recovery. Be professional and peer-to-peer.\nRecipient: {person_data.get('name')}, Title: {person_data.get('title')}, Company: {company_data.get('name')}, Industry: {company_industry}"""
...
def approve_and_enrich_contact(contact_id: int, supabase: Client = Depends(get_supabase)):
    # (Your existing approve_and_enrich_contact function remains unchanged)
    try:
        ...
        enriched_apollo_data, person_data_for_analysis = {}, person_data_from_db
        if apollo_person_id:
            apollo_response = get_apollo_person_details(apollo_person_id)
            if apollo_response and apollo_response.get('person'):
                enriched_apollo_data = apollo_response
        ...
        contact_email, photo_url = enriched_apollo_data.get('person', {}).get('email'), enriched_apollo_data.get('person', {}).get('photo_url')
        if enriched_apollo_data.get('person') and photo_url: enriched_apollo_data['person']['photo_url'] = photo_url
        update_data = {"email": contact_email, "apollo_person_data": enriched_apollo_data, "russia_ties_analysis": russia_analysis, "outreach_message": outreach_message_data, "status": 'Enriched'}
...
# **IMPROVEMENT: Refactored & Filterable Contact Endpoint**
@router.get("/", response_model=List[Contact])
def get_contacts(supabase: Client = Depends(get_supabase)):
    """
    Fetches all enriched contacts. This single endpoint replaces the previous
    /all and /campaigns endpoints, powering the People page.
    """
    ...
    for row in res.data:
        ...
        # Ensure a default campaign_status if it's null in the DB
...
# **IMPROVEMENT: Transactional Campaign Archiving**
...
def archive_campaign(req: ArchiveCampaignRequest, supabase: Client = Depends(get_supabase)):
    ...
    try:
        # Step 1: Create the past_campaigns entry. This is the first write operation.
        ...
        # Step 2: Create snapshots of the contacts for historical record.
        ...
        # Step 3: Update original contacts. They now point to the campaign name.
        ...
        # If any step after creating the campaign fails, attempt to roll back.
        if past_campaign_id:
...
def update_contact_message(contact_id: int, req: UpdateMessageRequest, supabase: Client = Depends(get_supabase)):
    # (Your existing update_contact_message function remains unchanged)
